...
Migration 3 deliberately tombstones pre-caller-intent `mm/v2/stage-request` receipts as `mm/v2/legacy-stage-request-conflict`. Their older digests cannot prove caller-intent equality, so they conflict rather than replay or fall back to remote resolution. Revise and cancel receipts are unchanged.
